...
export class Props {
    ...
    for (const lv of grid.levels) {
      for (const [ch, spec] of Object.entries(SPEC)) {
        for (const { tx, ty } of grid.find(lv.index, ch)) {
          ...
          if (spec.glow) {
            item.light = new THREE.PointLight(
              spec.glow,
              spec.glowI,
              spec.dist,
              1.8,
  ...
  selfCheck(grid) {
    ...
    for (const it of this.items) {
      ...
      if (above < -0.05 || above > slack)
        bad.push(`${it.spec.kind}@L${it.level} ${above.toFixed(2)}u`);
    }
    ...
    if (bad.length)
      console.warn('[props] off the ground or on the wrong level:', bad);
    return bad;
  }
  ...
  update(t, dt) {
    for (const it of this.items) {
      ...
      if (s.bob)
        it.mesh.position.y = it.base + Math.sin(t * 1.9 + it.phase) * s.bob;
      if (s.spin) it.mesh.rotation.y += s.spin * dt;
      if (it.light) {
        it.light.position.y = it.mesh.position.y + 0.35;
        it.light.intensity =
          s.glowI * (0.85 + Math.sin(t * 3.1 + it.phase) * 0.15);
      }
    }
  }
  ...
  wonder(level, x, z) {
    ...
    for (const it of this.items) {
      ...
      if (it.taken || it.sealed || it.level !== level || !it.spec.wonder)
        continue;
