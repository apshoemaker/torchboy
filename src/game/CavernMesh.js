  ...
  geometry(smooth) {
    ...
    if (smooth) {
      ...
      for (let i = 0; i < this.pos.length; i += 3) {
        ...
        let v = map.get(k);
        if (v === undefined) { v = verts.length / 3; map.set(k, v);
...
function slab(m, grid, tiles, topFn, bottomY, { inset = 0, sub = 1, waist = false } = {}) {
  ...
  const V = (cx, cy, band) => {
    ...
    const y = band === 2 ? topFn(cx, cy)
      : band === 1 ? bottomY + (topFn(cx, cy) - bottomY) * 0.52
      : bottomY;
    v = [x, y, z];
    cache.set(k, v);
    return v;
  ...
  for (const key of cells) {
    ...
    const c = [[cx, cy], [cx + 1, cy], [cx + 1, cy + 1], [cx, cy + 1]];
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (waist) {
        const wa = V(a[0], a[1], 1), wb = V(b[0], b[1], 1);
        m.quad(V(a[0],a[1],2), V(b[0],b[1],2), wb, wa);
        m.quad(wa, wb, V(b[0],b[1],0), V(a[0],a[1],0));
      } else {
        m.quad(V(a[0],a[1],2), V(b[0],b[1],2), V(b[0],b[1],0), V(a[0],a[1],0));
      }
    }
  }
}
...
function blob(m, cx, cy, cz, r, sy) {
  ...
  const p = [[cx, cy + r * sy, cz], [cx, cy - r * sy * 0.4, cz],
...
export function buildCavernLevel(grid, level, mats) {
  ...
  const floorAt = (tx, ty) => grid.floorHeight(level, ...(([x, z]) => [x, z])(centre(tx, ty)));

  const floors = [], near = new Set(), secretTiles = [];
  for (let ty = 0; ty < H; ty++) {
    for (let tx = 0; tx < W; tx++) {
      const ch = grid.char(level, tx, ty);
      if (ch === 'S') { secretTiles.push([tx, ty]); continue; }
      if (ch === '#' || ch === ' ') continue;
      floors.push([tx, ty]);
  ...
  slab(mFloor, grid, floors, topFloor, baseY - FLOOR_D, { sub: Grid.FLOOR_SUB });
  ...
  slab(mRock, grid, walls, topWall, baseY - FLOOR_D,
       { inset: 0.62, sub: WALL_SUB, waist: true });
  ...
  for (const [tx, ty] of floors) {
    ...
    if (grid.char(level, tx, ty) === '.' && hsh(tx, ty, 300 + level) < 0.055) {
      ...
      if (open >= 3) {
        ...
        cone(mDeco, x + off, y - 0.05, z - off, r, h, 6, (hsh(tx,ty,47)-0.5)*0.2);
      }
    }
    ...
    if (touching && hsh(tx, ty, 500 + level) < 0.30) {
      const n = 1 + Math.floor(hsh(tx, ty, 510) * 2);
      for (let k = 0; k < n; k++) {
        const r = 0.20 + hsh(tx, ty, 520 + k) * 0.36;
        blob(mDeco, x + (hsh(tx,ty,530+k)-0.5)*TS*0.75, y + r * 0.42,
             z + (hsh(tx,ty,540+k)-0.5)*TS*0.75, r, 0.62);
      }
    }
  }
  if (mDeco.count) {
  ...
  for (const { tx, ty } of grid.find(level, '*')) {
    ...
    for (let k = 0; k < 3; k++) {
      ...
      cone(mCry, x + Math.cos(a) * d, y, z + Math.sin(a) * d, r, h, 5,
...
export function cavernMaterials() {
  return {
    ...
    crystal: new THREE.MeshStandardMaterial({
      color: 0x388cb8, roughness: 0.25,
      emissive: 0x4db8f0, emissiveIntensity: 1.2,
    }),
  };
}
