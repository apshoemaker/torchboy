...
export class Ascension {
  ...
  update(dt, player) {
    ...
    const bright =
      t < APPROACH
        ? (t / APPROACH) * 0.35
        : t < BRIGHTEN
          ? 0.35 + ((t - APPROACH) / (BRIGHTEN - APPROACH)) * 0.65
          : 1;
    ...
    let dissolve = 0;
    if (t >= BRIGHTEN)
      dissolve = Math.min(1, (t - BRIGHTEN) / (DISSOLVE - BRIGHTEN));
    if (dissolve > 0) {
      ...
      for (let i = 0; i < MOTES; i++) {
        ...
        const local = Math.max(
          0,
          Math.min(1, (dissolve - s.delay) / (1 - s.delay)),
        );
      ...
      this.points.material.opacity =
        Math.sin(Math.min(1, dissolve * 1.15) * Math.PI) * 0.95;
    ...
    const whiteout =
      t < wStart ? 0 : Math.min(1, (t - wStart) / (WHITE - wStart));
  ...
  _fadeBody(player, amount) {
    player.root.traverse((o) => {
      ...
      for (const m of mats) {
        if (!m.__ascendPrepped) {
          m.transparent = true;
          m.depthWrite = false;
          m.__ascendPrepped = true;
        }
