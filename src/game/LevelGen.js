...
function validate(grid, isLast) {
  ...
  const floorTiles =
    ...
    find('>').length +
    find('X').length;
  if (floorTiles < 430 || floorTiles > 760)
    throw new Error(`floor=${floorTiles}`);
  ...
  if (opened.size !== secrets.length)
    throw new Error(
      `${secrets.length - opened.size}/${secrets.length} secrets unfindable`,
    );
  ...
  for (const r of rewards) {
    if (!open[r.y][r.x])
      throw new Error('reward unreachable even with secrets open');
...
export function generateLevels(seed = (Math.random() * 0xffffffff) >>> 0) {
  ...
  const levels = SPEC.map((s, i) => {
    ...
    for (let attempt = 0; attempt < 40; attempt++) {
      ...
      try {
        const { grid } = buildLevel(lvlSeed, {
          ...s,
          exit: i === SPEC.length - 1 ? 'X' : '>',
        });
