...
const COLD = new THREE.Color(0.7, 0.88, 1.0);
...
export class Torch {
   ...
   */
  get revealRadius() {
    return BASE_SENSE + EMBER_SENSE * Math.sqrt(this.embers);
  }

  /** How far out the flame can *feel* a passage - the hot/cold range. */
  get senseRadius() {
    return Math.min(this.revealRadius * 3.2, SENSE_CAP);
  }
  ...
  sense(nearest) {
    const r = this.revealRadius,
      s = this.senseRadius;
    this.proximity =
      nearest >= s ? 0 : nearest <= r ? 1 : 1 - (nearest - r) / (s - r);
  }
  ...
  update(dt) {
    ...
    const rate = 1 + p * 1.8;
    const n =
      Math.sin(this._t * 11.3 * rate) * 0.5 +
      Math.sin(this._t * 27.7 * rate) * 0.3 +
      (Math.random() - 0.5) * 0.4;
    const amp = 0.12 + p * 0.3;
    this._flick += (1 + n * amp - this._flick) * Math.min(1, dt * 16);

    const glow = 1 + this.embers * 0.1;
