...
class Tract {
  ...
  setShape(points, smooth = 3) {
    for (let i = 0; i < N; i++) {
      ...
      for (let p = 0; p < points.length - 1; p++) {
        if (x >= points[p][0] && x <= points[p + 1][0]) { a = points[p]; b = points[p + 1]; }
      }
      const span = b[0] - a[0];
    ...
    for (let s = 0; s < smooth; s++) {
      ...
      for (let i = 1; i < N - 1; i++) this.d[i] = (c[i - 1] + 2 * c[i] + c[i + 1]) / 4;
    }
  ...
  step(input) {
    ...
    for (let i = 0; i < N; i++) { R[i] = jR[i] * damp; L[i] = jL[i] * damp; }
    return R[N - 1];
  }
}
...
class VocalTractProcessor extends AudioWorkletProcessor {
  ...
  setVowel(v) {
    ...
    const hump = 0.95 + v * 0.40;
    this.tract.setShape([
      [0.00, 0.62],                        // glottal end stays narrow
      [0.12, 1.15],
      [0.32, pharynx],                     // open back cavity
      [0.60, hump],                        // tongue, back and low
      [0.82, 1.35],
      [1.00, lips],
    ]);
  }
  ...
  onMessage(m) {
    if (m.chord) {
      this.voices = m.chord.map((hz, i) => ({
        ...
        driftRate: 0.10 + Math.random() * 0.20,
  ...
  process(_inputs, outputs) {
    ...
    if (!out) return true;
    const n = out.length;
    const voiced = 1 - this.aspiration * 0.30;

    for (let s = 0; s < n; s++) {
      this.level += (this.targetLevel - this.level) * 0.0006;   // ~a second
      if (this.level < 1e-5 && this.targetLevel < 1e-5) { out[s] = 0; continue; }

      let glottal = 0, open = 0;
      ...
      const input = (glottal * voiced + breath * this.aspiration * 1.4) * this.level;
